...
#    Split by engine unit — critical to avoid
...
class EngineDataset(Dataset):
    def __init__(self, df, feature_cols, window=WINDOW_SIZE):
        ...
        for unit_id, grp in df.groupby("unit"):
            ...
            rul  = grp["RUL"].values
            # Slide window across each engine's history
...
def main():
    ...
    fd = "FD001"  # change to FD002, FD003, FD004 later
    ...
    # 3. Split training data into train / validation by engine unit
    ...
    evaluate(test_actuals, test_preds, "Bi-LSTM " + fd)

    # 6. Plots
    print("\nGenerating plots...")
    plot_loss_curves(train_hist, val_hist, fd)
    plot_predictions(test_actuals, test_preds, "Bi-LSTM", fd)
    plot_trajectory(test_proc, feature_cols, model, device, unit_id=1,  fd=fd)
